function [ desired_state ] = traj_generator3(t, state, waypoints)
...
%for e=1:(s(1,1)-1)
    ...
    if (t_index<=2)
        ...
        desired_state.vel = [traj_vel(x,1);traj_vel(x,2);traj_vel(x,3)];
        desired_state.acc = [traj_acc(x,1);traj_acc(x,2);traj_acc(x,3)];
        desired_state.yaw = 0;
        desired_state.yawdot = 0;
    end
