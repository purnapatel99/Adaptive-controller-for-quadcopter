...
for a=1:(s(1,1)-1)    
    ...
    for b=1:3
        if (a==1)
            ...
            vi=(waypoints0(a+1,b)-waypoints0(a,b))/(2*T);
        end
        if (a==s(1,1)-1)
            vf=0;
        else
            vf=(waypoints0(a+2,b)-waypoints0(a+1,b))/(2*T);
        end
        D=[waypoints0(a,b);waypoints0(a+1,b);vi;vf;0;0;0;0];
        C=inv(A)*D;
        if (a==1)
        traj(a,b)=C(1,1)*t^7+C(2,1)*t^6+C(3,1)*t^5+C(4,1)*t^4+C(5,1)*t^3+C(6,1)*t^2+C(7,1)*t+C(8,1);
        traj_vel(a,b)=C(1,1)*7*t^6+C(2,1)*6*t^5+C(3,1)*5*t^4+C(4,1)*4*t^3+C(5,1)*3*t^2+C(6,1)*2*t+C(7,1);
        traj_acc(a,b)=C(1,1)*42*t^5+C(2,1)*30*t^4+C(3,1)*20*t^3+C(4,1)*12*t^2+C(5,1)*6*t+C(6,1)*2;
        else
        traj(a,b)=C(1,1)*(t-des_t(a-1,1))^7+C(2,1)*(t-des_t(a-1,1))^6+C(3,1)*(t-des_t(a-1,1))^5+C(4,1)*(t-des_t(a-1,1))^4+C(5,1)*(t-des_t(a-1,1))^3+C(6,1)*(t-des_t(a-1,1))^2+C(7,1)*(t-des_t(a-1,1))+C(8,1);
        traj_vel(a,b)=C(1,1)*7*(t-des_t(a-1,1))^6+C(2,1)*6*(t-des_t(a-1,1))^5+C(3,1)*5*(t-des_t(a-1,1))^4+C(4,1)*4*(t-des_t(a-1,1))^3+C(5,1)*3*(t-des_t(a-1,1))^2+C(6,1)*2*(t-des_t(a-1,1))+C(7,1);
        traj_acc(a,b)=C(1,1)*42*(t-des_t(a-1,1))^5+C(2,1)*30*(t-des_t(a-1,1))^4+C(3,1)*20*(t-des_t(a-1,1))^3+C(4,1)*12*(t-des_t(a-1,1))^2+C(5,1)*6*(t-des_t(a-1,1))+C(6,1)*2;
